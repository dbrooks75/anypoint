...
On New or Updated File (C:\data\, AccountDeletes.csv)
  ...
  → For Each (loop over company names)
      ...
      → For Each (loop over BLAs — this loop already existed, with its own
          Salesforce Delete (Business_License_Application__c) at the end;
          the two new steps below just get inserted before that delete)
          → Set Variable (blaId)
      ...
      → Salesforce Query: SELECT Id FROM AccountContactRelation WHERE AccountId = ':accountId' AND IsDirect = false
          (Parameters: #[{accountId: vars.accountId}])
          (**corrected 2026-07-29**: `AND IsDirect = false` is required — `IsDirect = true` rows are
          the Contact's actual `AccountId` relationship, and Salesforce throws `INVALID_OPERATION`
          ("A direct relationship can't be deleted...") if you try to delete one directly; only
          reassigning `Contact.AccountId` or deleting the Contact itself removes it. Only the
          `IsDirect = false` rows this restructure explicitly creates for matched contacts are
          deletable this way.)
      ...
      → For Each (Collection: #[vars.candidateContactIds])
          ...
          → Salesforce Query: SELECT Id FROM AccountContactRelation WHERE ContactId = ':candidateContactId' AND AccountId != ':accountId'
              (Parameters: #[{candidateContactId: vars.candidateContactId, accountId: vars.accountId}])
              (**corrected 2026-07-29**: checks for relations to a genuinely *different* account,
              rather than "any remaining rows" — since `IsDirect = true` rows are never deleted above,
              a Contact solely owned by *this* account would still show its own direct relationship
              row and wrongly look "still shared" under the old "any remaining rows" check)
          → Choice
              When #[sizeOf(payload default []) == 0]:
                  → Set Variable: deletableContactIds = (vars.deletableContactIds default []) ++ [vars.candidateContactId]
              Otherwise: (2026-07-29 — still linked to another account via AccountContactRelation,
                  don't delete, but log it so it's visible in delete_log.csv)
                  → Set Variable: logEntries = (vars.logEntries default []) ++ [{
                        companyName: vars.companyName, object: "Contact", status: "Skipped - Still Linked",
                        salesforce_id: vars.candidateContactId, error_code: null,
                        error_message: "Contact still linked to another account via AccountContactRelation, not deleted"
                    }]
...
9. **AccountContactRelation** (new, 2026-07-28) — child of Account (`AccountId`) and Contact (`ContactId`), created by the load flow's lookup-before-create restructure (section 2's `AddContacts`/`AddContactsPetroleum`/`AddContactsBiWeeklyPayroll`, see section 10) whenever a candidate matched an existing Contact instead of creating a new one. Queried via `SELECT Id FROM AccountContactRelation WHERE AccountId = ':accountId' AND IsDirect = false` and deleted — this account's *indirect* relation to that Contact is gone regardless of whether the Contact itself gets deleted next. **`AND IsDirect = false` is required (2026-07-29)** — the `IsDirect = true` row is the Contact's actual `AccountId` relationship and can't be deleted directly (`INVALID_OPERATION: A direct relationship can't be deleted`); only reassigning `Contact.AccountId` or deleting the Contact removes it, which is exactly what step 10 below does when eligible.
10. **Contact, conditionally** (new, 2026-07-28) — this delete flow exists primarily to let the user reset test data while iterating, so unlike everything else in this list, Contact deletion isn't "always delete every child" — a Contact might be legitimately shared across multiple test accounts via `AccountContactRelation` (that's the whole point of the lookup-before-create restructure), and deleting it out from under another account would be wrong. Rule: query `Contact WHERE AccountId = ':accountId'` (this account's *directly*-owned Contacts, not ones only related via `AccountContactRelation`), then for each one, query `AccountContactRelation WHERE ContactId = ':candidateContactId' AND AccountId != ':accountId'` — a hit means this Contact is genuinely tied to some *other* account and must be skipped. Only Contacts with **zero** such rows get deleted. **Corrected 2026-07-29**: originally this checked for *any* remaining `AccountContactRelation` row at all, on the assumption step 9 already cleared this account's own rows — but step 9 can only delete `IsDirect = false` rows (Salesforce blocks deleting the `IsDirect = true` row that represents a Contact's actual `AccountId`, with `INVALID_OPERATION: A direct relationship can't be deleted`), so a Contact solely owned by *this* account would still show its own direct relationship row and never qualify under the old check. Filtering on `AccountId != ':accountId'` instead sidesteps this entirely — it doesn't matter whether step 9 could clear this account's own rows or not. **Skipped Contacts are logged too (2026-07-29)** — `status: "Skipped - Still Linked"`, `salesforce_id` set to the Contact's own Id even though nothing was deleted, so it's visible in `delete_log.csv` rather than silently disappearing from the run's output.
...
- **Salesforce Query bind parameters are substituted literally — no auto-quoting, no auto-escaping.** Verified empirically: dropping the quotes (`Name = :companyName`) produced `Name = Global Casting USA Inc` on the wire — an unquoted multi-word literal, not a properly quoted string. The connector does not add quotes for String parameters.
  ...
  - IDs (`accountId`, `invoiceId`, etc.) never need **escaping** — they're alphanumeric only.
  - **Correction (2026-07-29)**: IDs still need the **quotes** around the bind placeholder, though — confirmed empirically on the new `AccountContactRelation`/`Contact` delete queries (section 1), which failed until written as `AccountId = ':accountId'` rather than `AccountId = :accountId`. This project's existing Id-bound queries (e.g. this section's `Invoice__c WHERE Account__c = :accountId`, `Payment__c WHERE Invoice__c = :invoiceId`) are documented unquoted and were presumably working that way in Studio — **not yet rechecked** whether they actually need quotes too and just haven't been touched/retested since, or whether something specific to the new queries (the `!=` operator, or combining an Id compare with the `IsDirect = false` boolean literal) is what actually triggered this. Default to quoting every bind parameter, Id or not, until this is fully understood.
...
**Updated 2026-08-04, refined 2026-08-07**: a Mailing **and** a Physical Location are created for every row, in both Jewelry and Petroleum (`transform-location.dwl`/`transform-location-petroleum.dwl`) — **except** when the only address on file is a PO Box (one of `add1`/`add2` populated, the other blank, and the populated one contains "PO Box") — in that one case, only Mailing is created; there's no real street address to build a Physical Location from. The PO Box rule itself (which `Street` value each Location's Address__c gets) is unchanged and still lives entirely in `transform-address.dwl`'s `street` computation (which each Location's Address__c create reads via `vars.addressType`/`vars.locationName`):
- If `add1` and `add2` are both non-blank **and** one of them contains "PO Box", they are **not combined** — the PO-Box-containing line becomes the **Mailing** address's `Street`, and the other line becomes the **Physical** address's `Street`.
- Otherwise (no PO Box in either line, or only one line populated at all), Mailing and Physical get the **same** `Street` value (both lines concatenated if both populated, otherwise whichever single line is populated) — since there's no PO Box to separate out, the two addresses are identical. This branch no longer includes the single-line-PO-Box case (see below), since Physical isn't created there at all.

**Three-way outcome, as of 2026-08-07**:
1. Both lines populated, one is a PO Box → Mailing + Physical both created, different `Street` values (PO Box line vs. real street line).
2. Both lines populated, neither is a PO Box, **or** only one line populated and it's not a PO Box → Mailing + Physical both created, **same** `Street` value.
3. Only one line populated **and** it's a PO Box → **Mailing only** — no Physical Location, no Physical Address__c/PartyAddress__c, since there's no street address available at all for that row.

Before 2026-08-04, Physical Location was only created for case 1 above (cases 2 and 3 both got Mailing-only). The 2026-08-04 update made Physical unconditional for all three cases; this 2026-08-07 refinement pulled case 3 back out, since a lone PO Box shouldn't produce a Physical Location that's really just a duplicate of the PO Box itself.
...
| 28 | issue_date | Short Text (2026-07-21 — was Date/Time), non-padded `M/d/yyyy` per the `0:00:00`-export-artifact fix — see section 3's Date Format note |
| 29 | tot_pymt | |
| 30 | batchid | |
...
On New or Updated File (C:\data\, LoadReadyFlag.csv)
  ...
  → For Each row: (Collection: #[vars.laborStdRows])
      ...
      → Choice (wraps AddAccount/AddLocationsAndAddresses/AddContacts/AddBusinessLicenseApp below)
          When #[(vars.row.jobno default "") matches /^CS-.+/]: (proceed — see steps below)
          Otherwise: (skip row entirely — invalid jobno)
      ...
      → Flow Reference: AddBusinessLicenseApp
          ...
          → Choice
              When #[vars.blaId != null]:
                  ...
                  → Choice (**restructured 2026-08-14** — License creation gated on BLA Status;
                    Assessment/AQR moved out from under License's success path so they still run
                    when License is skipped)
                      When #[vars.blaStatus == "Approved"]:
                      ...
                      Otherwise: (skip — BLA is Draft, Business License/LicensePermitNameId not attempted)
...
**Per-row jobno-validity Choice — Studio-only step, not previously documented (found 2026-07-21)**: this Choice existed in Studio with the stale pre-`"CS-"` regex (`/^[1-9]\d*$/`), left over from before `jobno` became Text. Since it never matched the new `"CS-..."` values, every row silently skipped `AddAccount`/`AddLocationsAndAddresses`/`AddContacts`/`AddBusinessLicenseApp`, `blaJobnoLog` never got a single entry, and the `bla_jobno_map.csv` `File Write` (content = `vars.blaJobnoLog as CSV`) failed with `CSV Structure should be either an 'array<Object>' or an 'object' but got null` since the variable was never initialized (see the `null default []` note in the Result & Log Pattern section — that only helps once the first append actually runs). Fixed by updating the condition to `/^CS-.+/`, matching `transform1-filter-and-name.dwl`'s filter. Technically redundant now — `laborStdRows` is already filtered to `"CS-"`-prefixed rows before the `For Each` starts — but kept as a defensive per-row check rather than restructuring the flow. Confirmed working in Studio. **Lesson**: Studio-only Choice/Validation conditions that aren't backed by a `.dwl` file are invisible to a grep across `transforms/` — when a source field's format changes, check Studio GUI conditions too, not just the transform files.
...
### Date Format
`LaborAR.csv` date fields (`check_date`, `cash_pymt_date`, `mo_ord_date`, `deposit_date`) come through as `M/d/yyyy` — non-padded month/day (e.g. `1/1/2026`, `10/1/2025`, `1/10/2025`). Root cause: these Access columns were originally typed as Date/Time, which appended a `0:00:00` time component on CSV export (caused a real coercion error: `Text '8/23/2016 0:00:00'` failing against `MM/dd/yyyy`); fixed at the source by retyping the Access columns to Short Text before export, so the time component is gone but the month/day are still non-padded. All date parsing in `transform-invoice.dwl` and `transform-payment.dwl` uses `Date {format: "M/d/yyyy"}` accordingly — don't copy the `MM/dd/yyyy` pattern from the labor_std transforms into new AR-side code, and if a future Access export re-adds Date/Time typing, watch for this same issue resurfacing.

**`labor_std.csv`'s `issue_date` retyped to Short Text too (2026-07-21)** — previously the one exception still typed `MM/dd/yyyy` (padded, Date/Time column). Now retyped to Short Text like the AR fields above, so it hits the same `0:00:00`-artifact fix and comes through non-padded `M/d/yyyy` (e.g. `10/1/2025`) instead. Updated `transform-business-license.dwl` (`issueDateParsed`, `issueDateTime`), `transform-assessment.dwl` (`issueDateTime`), and `transform-partyaddress.dwl` (`Effective_From__c`) from `Date {format: "MM/dd/yyyy"}` to `Date {format: "M/d/yyyy"}` — single-letter `M`/`d` pattern letters parse leniently regardless of padding, so this is safe even if some rows still come in zero-padded. Confirmed working in Studio.
    ...
    → Flow Reference: AddBusinessLicenseAppPetroleum
        ...
        → Choice
            When #[vars.blaId != null]:
                ...
                → Choice (**restructured 2026-08-14** — License creation gated on BLA Status;
                  Assessment/AQR/ContentNote moved out from under License's success path so they
                  still run when License is skipped)
                    When #[vars.blaStatus == "Approved"]:
...
**Split flow (2026-08-04, revised 2026-08-17)** — one read + rename pass building all 285 fields per row, then two independent Transform Message steps each pick their subset of keys via `filterObject` and write their own CSV. `licenseno` is in both `file1Cols` and `file2Cols`, so it survives the filter into both outputs even though the source row only has it once. The read+rename step is now a tracked shared file, **`transform-trucks-raw-name.dwl`** (2026-08-17, reused for both `trucks_reg.unl`/`hitrucks.unl` — no `SourceFileType` parameter needed since neither file carries that column at all, unlike `transform-laborstd-raw-name.dwl`), which also:
- **Trims every field** (`trim(v default "")`, applied to all 285 columns uniformly, right after `pluck` and before the column-name mapping) — added 2026-08-17.
- **Strips the `.0` decimal artifact** from `licenseno` and every `year<N>` column (`needsStrip`/`stripDecimal`, same pattern as `padZip` elsewhere in this project) — added 2026-08-06/07.

The split (`file1Cols`/`file2Cols`) Transform Messages below are **not yet tracked as files** — still only documented here:
```
File Read: trucks_reg.unl (or hitrucks.unl for the Historical side — same flow, different file names)
  → Transform Message (transform-trucks-raw-name.dwl)
...
- **`transform-bla-petroleum.dwl`** — differs from `transform-bla.dwl` in several fields:
  - `Trade__c` — hardcoded to `"Petroleum"` (confirmed 2026-08-11, resolves dev question #9 — supersedes the earlier `"TBD"` placeholder).
  ...
  - `PrimaryOwnerId` (new field, 2026-07-16) = `vars.contactId` — the Id of the Contact created for this Account in `AddContactsPetroleum` (see that sub-flow's Flow Structure above for the new `Set Variable: contactId` step this required — Petroleum Contact creation didn't capture an Id at all before this, unlike other Creates in this project).
  - `SiteAddress` (new field, 2026-07-17, compound **Address**-type field) = the Mailing address. Reuses `transform-address.dwl`'s PO-Box-detection logic (`hasPOBox1`/`hasPOBox2`/`bothPopulated` on `add1`/`add2`) to resolve the street value, always taking the `isMailing == true` branch since there's no per-location `addressType` variable here (this transform runs once per row, not once per Location item).
  - **Correction (2026-07-17)**: first tried a nested object (`SiteAddress: { street, city, state, postalCode, country }`) — threw `SALESFORCE:INVALID_INPUT`. Root cause: a custom **Address**-type field's compound name (`SiteAddress__c`) is **read-only** — Salesforce auto-composes it from separate flat shadow fields suffixed `__s`, not `__c`, which is what actually has to be populated on Create.
  - **Correction #2 — the `__s` suffix assumption was wrong.** Confirmed via Workbench (Info → Standard & Custom Objects → `Business_License_Application__c` → Fields → `SiteStreet` → **Name** column, not **Label**; Object Manager's field detail page didn't show the breakdown at all) that the real API names have **no suffix whatsoever** — `SiteStreet`, `SiteCity`, `SiteState`, `SiteStateCode`, `SitePostalCode`, `SiteCountry`, `SiteCountryCode`. This actually mirrors how Salesforce's own **standard** compound Address fields work (e.g. `MailingStreet`/`MailingCity`/`MailingStateCode` on Contact/Lead — prefix + component name, no suffix at all), not the custom-field `__ComponentName__s` shadow-field pattern assumed in Correction #1 above — that assumption turned out not to apply to this field, and was corrected once the real Workbench describe data was checked.
  - Both the plain-text (`SiteState`/`SiteCountry`) **and** picklist-code (`SiteStateCode`/`SiteCountryCode`) variants exist side by side — confirming State and Country Picklists are enabled org-wide. When both exist, the **Code** fields are the ones to populate (picklist-validated, authoritative); the plain-text fields are derived/read-only from them. Final implementation: `SiteStreet`, `SiteCity`, `SiteStateCode` (`vars.row.state`, already a 2-letter code), `SitePostalCode`, `SiteCountryCode: "US"` (2-letter ISO, not `"United States"` — the free-text form only applies to the non-Code variant).
- **`transform-business-license-petroleum.dwl`** — replaces `transform-business-license.dwl`; `date_issued` instead of `issue_date`, `licenseno` instead of `jobno` throughout (`Name: "PET-" ++ licenseno` — confirmed prefix is `"PET-"`, not Jewelry's `"CS-"`, `Legacy_License_Number__c: licenseno`).
  - **`Status` rule change (2026-07-17), supersedes the original `SourceFileType`-only Active/Inactive rule**: `SourceFileType == "Historical"` → always `"Expired"` (was `"Inactive"`). `SourceFileType == "Current"` → `"Active"` if a `2026` `deposit_date` exists in `MercAR` for this `licenseno` (reuses the same `hasCurrentYearDeposit` var `Expiration_Date__c`/`PeriodEnd` already compute), else `"Expired"` (was always `"Active"` regardless of AR).
  - **`Expiration_Date__c`/`PeriodEnd` rule change (2026-07-15)**: no longer derived from `date_issued` (was: last day of the month before issue-month, +1 year — Jewelry's rule, originally copied over for Petroleum too). Was: 7/31 of the year after `license_issued`, e.g. `license_issued` `2026` → expiration `7/31/2027`.
  - **`Expiration_Date__c`/`PeriodEnd` rule change #2 (2026-07-17), supersedes the above**: now branches on `SourceFileType`, and **no longer depends on `license_issued` at all for Current records**. If `SourceFileType == "Current"`: `7/31/2027` if a `2026` `deposit_date` exists in `MercAR` for this `licenseno` (same `matchingArRows`/2026-year-check pattern already used in `transform-bla-petroleum.dwl`'s Status/ApplicationType logic — duplicated here since each transform file is standalone, no shared variables across files), else `7/31/2026`. If `SourceFileType == "Historical"`: unchanged from rule #1 — `7/31` of `license_issued` + 1 year.
  ...
  - **`PeriodStart`/`Issue_Date__c` year rule change (2026-07-28), supersedes the above for Current-sourced rows**: `Current` no longer derives the year from `license_issued` at all — now **`2026`** if `hasCurrentYearDeposit` (a `2026` `deposit_date` exists in `MercAR` for this `licenseno`, same check `Expiration_Date__c` already uses), else **`2025`**. Mirrors `Expiration_Date__c`'s Current-branch pattern exactly one year earlier each time, so the Aug-to-Jul one-year window still lines up: `hasCurrentYearDeposit` → PeriodStart `8/1/2026`/PeriodEnd `7/31/2027`; no current-year deposit → PeriodStart `8/1/2025`/PeriodEnd `7/31/2026`. **`Historical`-sourced rows are unchanged** — still `license_issued`'s own year. New `periodStartYear` var holds the resolved year (either the hardcoded `2026`/`2025` for Current, or `licenseIssuedYear` for Historical); `periodStartDateTime`/`periodStartDate` now build off it instead of `licenseIssuedYear` directly.
...
**Import procedure (confirmed 2026-08-06)** — unlike Jewelry/Petroleum, the source file arrives as a **secured Excel file** that Access can't import directly (the file's protection blocks a direct Excel import). Workaround: open it, **Save As `.csv`**, then in Access use **External Data → Text File** (not the Excel import path) to bring it in. During that import, set **`DateRecd`** to **Short Text**, not the default DateTime type Access would otherwise assign — same reasoning as the already-confirmed `0:00:00` Access export artifact fix (see the "hit the same Access `0:00:00` export artifact" note for `DateRecd`/`DateApproved`/etc. further down this section) — doing it at import time avoids having to retype the column after the fact.
...
**`AddressType`/`Address_Type__c` (confirmed 2026-07-13, revised 2026-08-04)**: originally all Locations got the placeholder `"Physical"` ("that may change later," per the user) since `AddressType` wasn't yet distinguishing anything — `Location.Name`/`vars.locationName` alone drove which source columns fed the Address. **Now that a real "Physical Location" exists, `AddressType` is a genuine three-way value**: `"Mailing"` for Company, `"Physical"` for Physical Location, `"Corporate"` for Corporate — **`"Corporate"` is a valid picklist value on this org** (correction — an earlier note in this doc, based on Jewelry/Petroleum's Mailing/Physical-only usage, wrongly assumed it wasn't). Computed **inside each transform** from `vars.locationName` (`transform-address-biweeklypayroll.dwl` and `transform-partyaddress-biweeklypayroll.dwl` each have their own identical local `addressType` var), not read from an externally-set `vars.addressType` — the Studio-side `Set Variable: addressType` step is no longer needed at all, see Flow Structure below.
...
- `transform-address-biweeklypayroll.dwl` — **revised 2026-08-04**: branches `Street`/`City`/`StateCode`/`PostalCode` on `vars.locationName == "Corporate"` (→ `CorpOffice*` columns) vs. else (→ `Company*` columns) — inverted from the original `isCompany` check, since two of the three Location names ("Company", "Physical Location") now share the same source columns rather than just one. `AddressType` is a local `addressType` var (`"Mailing"`/`"Corporate"`/else `"Physical"`, computed from `vars.locationName` right in this transform); `Country: "United States"` (confirmed, not `CountryCode`).

**`transform-partyaddress-biweeklypayroll.dwl` confirmed (2026-07-13, `Address_Type__c` revised 2026-08-04) and built**: `Effective_From__c` derives from `DateRecd` (`M/d/yyyy` format, confirmed correct). `Is_Primary__c` is `vars.locationName == "Company"` (Company is primary; Physical Location and Corporate are not — unaffected by the 2026-08-04 Physical Location addition, since the check is exact-match on `"Company"`). `Address_Type__c` uses the same local `addressType` computation as `transform-address-biweeklypayroll.dwl` above, not an externally-set `vars.addressType`. `PartyId`/`AddressId__c` unchanged from the existing pattern.
...
Flow Reference: AddLocationsAndAddressesBiWeeklyPayroll
  ...
  → For Each location result: (Collection: #[vars.locationResults])
      ...
      → Set Variable: locationName = #[payload.locationName]
          (no separate addressType Set Variable step, 2026-08-04 — both transforms below compute
          it themselves from vars.locationName, see AddressType note above)
      → Choice
...
**RESOLVED 2026-07-27** (originally flagged 2026-07-21): every DateTime field built via the `(dateString ++ "T00:00:00Z") as DateTime {...}` pattern anchored at **midnight UTC**, which could display as the **previous calendar day** for any viewer/org timezone behind UTC — Salesforce DateTime fields are stored UTC and rendered in the viewer's timezone. Surfaced by the `AppliedDate: |1900-01-01T00:00:00Z|` placeholder in `transform-bla.dwl`/`transform-bla-petroleum.dwl` showing as `12/31/1899` in the app (an 8-hour-behind timezone rolling midnight UTC back a day) — but the same mechanism applied to every real-date DateTime field built this way too. Fixed by switching every occurrence to anchor at **noon UTC** (`T12:00:00Z`) instead of midnight — safe for any timezone offset within ±12 hours, which covers every US timezone. Scope decided as **all 12 files** (both the dynamic `(dateStr ++ "T00:00:00Z")` construction sites and the two hardcoded `1900-01-01` placeholders), applied across `transform-bla.dwl`, `transform-bla-petroleum.dwl`, `transform-bla-biweeklypayroll.dwl`, `transform-business-license.dwl`, `transform-business-license-petroleum.dwl`, `transform-business-license-biweeklypayroll.dwl`, `transform-assessment.dwl`, `transform-assessment-petroleum.dwl`, `transform-assessment-biweeklypayroll.dwl`, `transform-partyaddress.dwl`, `transform-partyaddress-petroleum.dwl`, `transform-partyaddress-biweeklypayroll.dwl` — 16 occurrences total, all now `T12:00:00Z`.
...
1. **`dataType` is queried from Salesforce**, not hardcoded per question — `AssessmentQuestionVersion.DataType` is a real field (already confirmed to exist, see section 7's BiWeeklyPayroll Version-1-vs-2 pin note). **Applied in Studio (2026-07-27)**: added to the `SELECT Id, QuestionText, Name, VersionNumber FROM AssessmentQuestionVersion` query in all three `InitAssessmentQuestionVersion*` sub-flows (section 2, section 6, section 7) → `SELECT Id, QuestionText, Name, VersionNumber, DataType FROM AssessmentQuestionVersion`.
...
| `license` | `transform-elevator-license-raw-name.dwl` (Current only — see `hi_license.unl` exception below) + `transform-elevator-license-historical-raw-name.dwl` (Historical only) | `transform-elevator-license-combine-export.dwl` |
...
**`hi_comp_lic.unl` schema exception, confirmed 2026-08-26 — same class of bug**: `comp_lic`'s Historical source file has **29 columns, not 30** — missing `license_ai` (originally column 3). Previously `transform-comp-lic-raw-name.dwl` was shared by both `comp_lic.unl` (Current) and `hi_comp_lic.unl` (Historical); it's now Current-only, and Historical gets its own dedicated `transform-comp-lic-historical-raw-name.dwl`, built field-by-field with `license_ai: null` at its original position (3) — same fix shape as `transform-elevator-pr-raw-name.dwl` above, applying the same corrected key-order lesson.

**`hi_license.unl` schema exception, confirmed 2026-08-26 — same bug again**: `license`'s Historical source file also has **32 columns, not 33** — also missing `license_ai` (originally column 3), same as `hi_comp_lic.unl`. Split the same way: `transform-elevator-license-raw-name.dwl` is now Current-only, and `transform-elevator-license-historical-raw-name.dwl` handles Historical with `license_ai: null` at position 3.

**Checked and confirmed fine, 2026-08-26**: `hi_payments.unl` and `hist_viol.unl` (Historical files for `payments`/`violation`) match their Current counterparts' column counts — no `license_ai`-style gap on those two. So the pattern is specifically a `license_ai` gap on the two tables that have that column (`comp_lic`, `license`), not a general Historical-vs-Current risk across every entity.
...
| `license` | `AddLicenseFiles` | 2-way (same as `comp_lic`) — **Historical `Try` uses `transform-elevator-license-historical-raw-name.dwl` + its own 32-col `expectedCols`, not the Current transform**, per the `hi_license.unl` exception above | `license.unl` / `hi_license.unl` | `currentLicenseRows` / `historicalLicenseRows` |
...
**`AddElevatorFiles`'s Private `Try` block is the one exception to the "same transform, different `sourceFileType`" pattern**: its Current/Historical branches call `transform-elevator-raw-name.dwl` (72-column `vars.expectedCols`), but its Private branch must call `transform-elevator-pr-raw-name.dwl` instead, with its own 70-column `vars.expectedCols` (no `device_active`/`insp_date_due`) for the column-count check — see the `hi_elevator_pr.unl` schema exception note above for why a shared transform doesn't work here.
