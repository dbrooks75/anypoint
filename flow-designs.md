...
- **`transform-contentnote-petroleum.dwl`** (new file, 2026-07-15) — builds `ContentNote` (standard object, no `__c`): `Title: "Petroleum Conversion"`, `Content` = Base64-encoded HTML string built from `vars.row` (`certificate`, `certif_dmv`, `certif_gu`, `comments`), 4 `<p>` tags — `"Certificate: " ++ certificate`, `"Certificate (DMV): " ++ certif_dmv`, `"Certificate GU: " ++ certif_gu`, `"Comments: " ++ comments`. Base64 encoding done via `dw::core::Binaries::toBase64` (`import toBase64 from dw::core::Binaries`, then `toBase64(html)`). **Two corrections en route (2026-07-15)**: (1) first tried the `String`↔`Binary` coercion form (`html as Binary {encoding: "UTF-8"} as String {encoding: "base64"}`) — failed, this runtime's coercion treats `encoding` as a strict Java charset name only, not the newer DataWeave base64/hex special values (`Cannot coerce Binary to String ... unsupported charset: 'base64'`); (2) then tried `write(html, "application/base64") as String` to sidestep the coercion — also failed, "unknown content type", meaning the `application/base64` writer module isn't registered in this runtime either. `toBase64` finally worked because it's a plain DataWeave function (not a MIME reader/writer extension), so it doesn't depend on any writer module being present — it only needed the explicit `import`, unlike the unqualified `some` call noted below which was never given one. **Takeaway**: in this project's DataWeave runtime, prefer an explicit `import` from a `dw::core::*` module over relying on an unregistered MIME writer/reader — don't assume a `write()`/`read()` content-type trick is more "core" than a stdlib function import.
