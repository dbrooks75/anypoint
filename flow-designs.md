...
**Updated 2026-08-04**: a Mailing **and** a Physical Location are now always both created for every row, in both Jewelry and Petroleum (`transform-location.dwl`/`transform-location-petroleum.dwl`, no more PO-Box-conditional branch — always returns both). The PO Box rule itself is unchanged and still lives entirely in `transform-address.dwl`'s `street` computation (which each Location's Address__c create reads via `vars.addressType`/`vars.locationName`):
...
- Otherwise (no PO Box in either line, or only one line populated at all), Mailing and Physical get the **same** `Street` value (both lines concatenated if both populated, otherwise whichever single line is populated) — since there's no PO Box to separate out, the two addresses are identical.

Previously, Physical Location was only created when the PO-Box condition triggered it; a row with no PO Box (single-line address, or two non-PO-Box lines) got only a Mailing Location and no Physical Location at all. That's the behavior this update replaces — Physical is now unconditional, so the two Locations end up address-identical in the no-PO-Box case rather than one being missing.
